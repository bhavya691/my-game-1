...
var arrowgroup;
var health = 1000;
var collide , invisibleBlock , reset , resetImage;
var Play = 1;
var End = 0;
var gameState = Play;
...
function setup(){
  ...
  invisibleBlock = createSprite(enemy.x-52 , enemy.y , 50  ,10);
...
if(gameState === Play){
  ...
  spawnArrows();
  //collide
  if(arrow.body.position.x >= invisibleBlock.x && arrow.body.position.y >= invisibleBlock.y){
    enemy.x = random(displayWidth-1000 , displayWidth-100);
    enemy.y = random(displayHeight-600 , displayHeight-300);
    invisibleBlock.x = enemy.x;
    invisibleBlock.y = enemy.y;
    platform.x = enemy.x;
